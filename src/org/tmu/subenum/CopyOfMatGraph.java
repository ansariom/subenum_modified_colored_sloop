...
public class CopyOfMatGraph implements Graph {
...
//    public boolean[] adjArr;
     ...
     * @throws IOException
     */
    public static CopyOfMatGraph readColoredGraph(Reader reader) throws IOException {
    	BufferedReader br = new BufferedReader(reader);
    	String line;
    	CopyOfMatGraph graph = new CopyOfMatGraph();
    	...
    	while ((line = br.readLine()) != null) {
    		...
    		vColorHash.put(src, color1);
    		vColorHash.put(dest, color2);
    	}
    	br.close();
    	graph.update();
    	return graph;
    ...
    public static CopyOfMatGraph readGraph(Reader reader) throws IOException {
        BufferedReader br = new BufferedReader(reader);
        String line;
        CopyOfMatGraph graph = new CopyOfMatGraph();
...
//    }
     ...
     * @throws IOException
     */
    public static CopyOfMatGraph readStructure(Reader reader) throws IOException {
...
//                map.put(tokens[1], last_v++);
    		...
    		byte color1 = Byte.parseByte(tokens[2]);
    		byte color2 = Byte.parseByte(tokens[3]);
    		vColorHash.put(Integer.valueOf(tokens[0]), color1);
    		vColorHash.put(Integer.valueOf(tokens[1]), color2);
    		graph.addEdge(src, dest);
//            graph.addEdge(map.get(tokens[0]), map.get(tokens[1]));
        }
        br.close();
    ...
    public static CopyOfMatGraph readFromFile(String path) throws IOException {
//    	return readGraph(new FileReader(path));
        return readColoredGraph(new FileReader(path));
    }

    public static CopyOfMatGraph readStructureFromFile(String path) throws IOException {
...
//    	adjArr = new boolean[table.size() * table.size()];
    ...
    /** Removed by Mitra because of duplication**/
//    final private void update() {
//        edgeCount = 0;
//        adjArr = new boolean[table.size() * table.size()];
//        for (int v : vertices) {
